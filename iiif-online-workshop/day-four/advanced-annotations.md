# Advanced Annotations

There is unlikely to be time to cover this in the session but if your interested in other ways of doing annotation there is this [google slides](https://docs.google.com/presentation/d/1StVt_Eixt9wmFXoyPRjjlq-J1VevR7b4qvVmVs7yBWg/edit?usp=sharing) presentation. The links for the different examples are available below:

 * [Video on how to configure the crowdsourcing system](https://www.youtube.com/playlist?list=PLMd2mmRYjSJlKs829X0z_kYueQemSfwDd)
 * [Recogito](https://recogito.pelagios.org/)
 * [From the Page](https://fromthepage.com/)
 * [Indigenous Digital Archive](https://digirati.com/showcasing-collections/indigenous-digital-archive-treatise-explorer/)

One of the real promises of IIIF is that once you have interoperable Images and presentation packages (Manifests) it should be easy to then take these and use them in these advanced tools. In practice once you have IIIF Images and Manifests you should be able to take them and run a crowdsourcing project using one of the tools above. 
 
Other examples of annotation tools with IIIF:

 * [Allmaps](https://allmaps.org/) - Georeference IIIF Maps
 * [liiive](https://liiive.now/) - realtime collaborative annotation editor (see [guest presentation](https://training.iiif.io/iiif-online-workshop/GuestPresentations.html#liiive))
 * [Exhibit](https://exhibit.so/) - story telling annotation tool (see [guest presentation](https://training.iiif.io/iiif-online-workshop/GuestPresentations.html#exhibit))
 * [Madoc](https://madoc.digirati.com/) - a flexible crowdsourcing, research and teaching platform for digitised collections.
 * [Glycerine Workbench](https://glycerine.io/) – A scholarly annotation and publication workbench. Glycerine provides end-to-end workflows for collaboration and publishing of images. Glycerine is a comprehensive framework for institutional integration.

See more examples on the IIIF Awsome list: https://github.com/IIIF/awesome-iiif#annotations 
